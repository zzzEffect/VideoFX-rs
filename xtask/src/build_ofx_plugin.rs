...
fn build_plugin_for_target(target: &Target, release_mode: bool) -> std::io::Result<PathBuf> {
    ...
    let mut cargo_args: Vec<_> = vec![
        String::from("build"),
        String::from("--package=video-fx-openfx-plugin"),
        String::from("--lib"),
        String::from("--target"),
    ...
    let mut built_library_path =
        target_dir_path.plus(target.library_prefix.to_owned() + "video_fx_openfx_plugin");
...
pub fn main(args: &clap::ArgMatches) -> Result<(), Box<dyn Error>> {
    ...
    let plugin_bundle_path = output_dir.plus_iter(["VideoFx.ofx.bundle", "Contents"]);
    let plugin_bin_path = plugin_bundle_path.plus_iter([ofx_architecture, "VideoFx.ofx"]);
