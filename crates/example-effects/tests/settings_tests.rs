use example_effects::settings::color_adjustment::{setting_id, ColorAdjustmentFullSettings};
use example_effects::settings::{
    EnumValue, GetSetFieldError, SettingKind, Settings, SettingsBlock, SettingsList,
};

#[test]
fn settings_list_has_all_descriptors() {
    let list = SettingsList::<ColorAdjustmentFullSettings>::new();
    ...
    // 7 descriptors: brightness, invert_colors, tint (ColorRGB), advanced (group),
    // contrast (child), saturation (child), color_preset
    assert_eq!(all.len(), 7);
}
...
fn json_round_trip() {
    ...
    settings.color_preset =
        example_effects::settings::color_adjustment::ColorPreset::Warm;
    ...
    assert_eq!(parsed.color_preset, example_effects::settings::color_adjustment::ColorPreset::Warm);
}
...
fn type_mismatch_error() {
    let settings = ColorAdjustmentFullSettings::default();
...
fn legacy_value_is_default() {
    let legacy = ColorAdjustmentFullSettings::legacy_value();
    let default = ColorAdjustmentFullSettings::default();
    assert_eq!(legacy.brightness, default.brightness);
}

#[test]
fn descriptor_labels() {
    let descriptors = ColorAdjustmentFullSettings::setting_descriptors();
